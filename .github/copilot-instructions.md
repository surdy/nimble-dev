...
10. **Install live examples in the local app config directory after every new feature.** After implementing a new action type or config feature, add a working example directly to `~/Library/Application Support/com.ctx.launcher/` (macOS) so it is immediately testable in the running app. Mirror the same files that were added to `example-config/`.
