...
- **Copilot skill over agents for command/script authoring** — replaced two separate agents (`nimble-command`, `nimble-script`) and a conventions file with a single `nimble-authoring` skill; eliminates phantom agent in picker, removes prompt pollution, removes spec_version sync dance, enables end-to-end authoring in one pass _(2026-03-25)_
