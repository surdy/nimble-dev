...
Runs an external script and displays its output as a list of items the moment the phrase is matched. Like `static_list`, no `Enter` is required to expand the list. Scripts can optionally accept a typed argument to filter or parameterise their output in real time.
...
Each script writes to stdout and Ctx parses the result:

| Output type | How Ctx interprets it |
|-------------|----------------------|
...
# List appears on exact match; re-runs with suffix as the script re-filters
